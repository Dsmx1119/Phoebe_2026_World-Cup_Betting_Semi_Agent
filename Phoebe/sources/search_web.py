...
class SearchWebAdapter(SourceAdapter):
    source_name = "search_web"

    def collect(self, match: MatchContext, settings: Settings) -> SourceResult:
        query = f"{match.home_team} {match.away_team} football preview odds lineups"
        ...
        try:
            ...
            snippets = [text[:800]] if text else []
        return SourceResult(
            source_name=self.source_name,
            source_url=url,
            accessed_at=utc_now_iso(),
            data={"query": query, "snippets": snippets, "status": "available"},
            confidence="low",
            notes="Public search result snippets only; verify primary sources before stronger claims.",
        )
