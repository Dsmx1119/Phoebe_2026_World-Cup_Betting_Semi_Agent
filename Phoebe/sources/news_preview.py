...
class NewsPreviewAdapter(SourceAdapter):
    source_name = "news_preview"

    def collect(self, match: MatchContext, settings: Settings) -> SourceResult:
        query = f"{match.home_team} vs {match.away_team} expected lineups injuries preview"
        ...
        return SourceResult(
            ...
            confidence="low",
            notes="Search page fetched, but MVP does not promote snippets into factual lineup claims without extraction.",
        )
